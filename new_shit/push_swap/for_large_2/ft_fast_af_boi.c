...
void	ft_fast_af_boi(t_stack **a, t_stack **b, t_pack *pack)
{
	...
	ft_print_stacks(*a, *b, "ads", *pack);
	if (ft_pick_rotate(aptr, ft_get_max(aptr)) == 1)
		while (aptr->value != ft_get_max(aptr))
		{
			ft_reverse_rotate_a(a, pack);
			aptr = *a;
		}
	...
	aptr = *a;
	bptr = *b;
	while (bptr->value != ft_get_max(bptr) && bptr->value >= pivots->value)
	{
		ft_rotate_b(b, pack);
		bptr = *b;
	}
